...
export const getArticlePageData = (id: string) => {
  ...
  return {
    ...
    tags: PostsManager.getCountsGroupByTags().slice(0, 50),
    dates: PostsManager.getCountsGroupYearMonth(),
    recommends: PostsManager.getRecommendsPosts(post.tags, 15).filter((item) => item.path !== post.path),
  };
};
...
export const getTagPageData = (tag: string, page: number) => {
  ...
  return { posts: allPosts.slice((page - 1) * POSTS_PER_PAGE, page * POSTS_PER_PAGE), pageCount };
};

export const getTagParams = () => PostsManager.getAllTagNames().flatMap((tag) => {
  const pageCount = Math.ceil(PostsManager.findByTag(tag).length / POSTS_PER_PAGE);
  return Array.from({ length: pageCount }, (_, index) => ({ tag, page: String(index + 1) }));
});

export const getDateParams = () => PostsManager.getCountsGroupYearMonth();
export const getAllPostIds = () => PostsManager.getData().map((post) => ({ id: post.path.split('/').pop() as string }));
