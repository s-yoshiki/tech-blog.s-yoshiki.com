...
const TableOfContents = ({ items }: { items: string[] }) => (
  <aside className="hidden lg:block">
    <div className="sticky top-24 rounded-2xl border bg-card p-6 shadow-sm">
      <h2 className="mb-4 text-sm font-bold uppercase tracking-wider">目次</h2>
      <ol className="space-y-2 border-l text-sm">
        {items.map((item) => (
          <li key={item}>
            <a
              className="block border-l-2 border-transparent py-1 pl-4 text-muted-foreground transition hover:border-primary hover:text-primary"
              href={`#${item}`}
            >
              {item}
            </a>
...
const ArticleCollection = ({
  ...
  posts.length ? (
    <section className={`border-t py-16 ${muted ? 'bg-card' : ''}`}>
...
export default function ArticlePage({
  ...
  return (
    <article className="pt-12 sm:pt-16">
      <header className="mx-auto mb-12 max-w-4xl px-4 text-center sm:px-6">
        <div className="mb-6 flex flex-wrap justify-center gap-2">
          {post.tags.slice(0, 5).map((tag) => (
            <Link href={`/tags/${tag}/1`} key={tag}>
              <Badge keyword={tag} />
            </Link>
          ))}
        </div>
      ...
      <div className="mt-20">
        <section className="border-t bg-card py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6">
            ...
            <SectionHeading>おすすめの記事</SectionHeading>
            <PostsBand posts={recommends} />
          </div>
        </section>
        <ArticleCollection title="最新の記事" posts={latest} />
        <ArticleCollection title="よく読まれている記事" posts={popular} muted />
        <section className="border-t py-16">
          <div className="mx-auto grid max-w-7xl gap-12 px-4 sm:px-6 md:grid-cols-[1fr_2fr]">
            <div>
