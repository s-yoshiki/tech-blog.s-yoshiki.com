...
100問は扱うアルゴリズムこそ異なりますが、Canvas上の実装はおおむね次の流れで読めます。

1. `ImageData`からRGBAの一次元配列を取得する
2. `(x, y)`を`(y * width + x) * 4`へ変換する
3. 入力とは別の配列へ計算結果を書き込む
4. 値を0〜255へ丸め、`putImageData`で表示する

処理中に入力配列を直接書き換えると、走査済みの画素が後続の計算へ混ざります。畳み込み、
モルフォロジー、反復型アルゴリズムでは、入力と出力のバッファを分けることが再現性の鍵です。

また、端の画素を無視するのか、0で埋めるのか、端の値を複製するのかによって結果が変わります。
デモの出力だけでなく、座標の範囲判定とクランプ処理まで読むと、アルゴリズムを別の画像や
ライブラリへ移植しやすくなります。

## 学習時の検証方法

最初から写真だけで確認せず、全画素が同じ画像、中央だけ白い画像、横方向のグラデーションなど、
結果を予測できる小さな入力を用意するのがおすすめです。中間画像やヒストグラムも表示すると、
「数式が間違っている」のか「座標や配列添字がずれている」のかを切り分けられます。
